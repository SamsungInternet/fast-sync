...
function getWebSocketServer(server, options) {
	...
	wss.on('connection', function connection(ws) {
		...
		ws.on('close', function close() {
			...
			if (!!options.debug) console.log('user with id', id, 'left room', ws._room);
			rooms[ws._room].removeWs(ws);
		});
		...
		ws.on('message', function incoming(message) {
			...
			if (typeof message === 'string') {
				...
				if (data[0] === 'HANDSHAKE') {
					if (!!options.debug) console.log('user with id', id, 'joined room', data[1]);
	...
	wss.dist.use(options.path, express.static(path.resolve(path.join(__dirname, '../', 'dist'))));
